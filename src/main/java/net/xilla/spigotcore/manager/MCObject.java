...
//            }
            ...
            while (clazz.getSuperclass() != null && !clazz.getSuperclass().getName().equals("MCObject")) {
                for (Field field : clazz.getDeclaredFields()) {
                    if (field.getAnnotation(StoredData.class) != null) {
                        ...
                        if (object instanceof SerializedObject) {
                            ...
                            json.put(field.getName(), ((Material) object).name());
...
//        }
        ...
        Class<?> clazz = getClass();
        while (clazz.getSuperclass() != null && !clazz.getName().equals("MCObject")) {
            for (Field field : clazz.getDeclaredFields()) {
                if (field.getAnnotation(StoredData.class) != null) {
                    try {
                        if(!field.isAccessible()) {
                            ...
                            if(field.getType().isAssignableFrom(SerializedObject.class)) {
                                ...
                                field.set(this, Location.deserialize(json.get(field.getName())));
                            } else if(field.getType().isAssignableFrom(Material.class)) {
                                field.set(this, Material.valueOf(json.get(field.getName())));
                            } else {
                                field.set(this, json.get(field.getName()));
                            }
